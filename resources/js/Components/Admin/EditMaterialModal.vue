<template>
  <div class="edit-modal" @click.self="$emit('close')">
    ...
    >
      ...
      <h2 class="text-xl">Modifier un matériau</h2>
...
export default {
  ...
  methods: {
    update() {
      axios
        .put(route("admin.materials.update", this.material.id), this.form)
        .then((response) => {
          this.$emit("updateMaterialsList", response.data);
          this.$emit('close');
          this.$emit('success', 'Matériau mis à jour avec succès !');
        })
        .catch((error) => {
